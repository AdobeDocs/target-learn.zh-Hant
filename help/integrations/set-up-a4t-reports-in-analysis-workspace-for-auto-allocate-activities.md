...
[[!UICONTROL Auto-Allocate]中的](https://experienceleague.adobe.com/docs/target/using/activities/auto-allocate/automated-traffic-allocation.html?lang=zh-Hant){target=_blank}活動[!DNL Adobe Target]會從兩個或多個體驗中識別獲勝者，並自動重新分配訪客流量給獲勝者，同時測試會繼續執行和學習。 [!UICONTROL Analytics for Target]的[!UICONTROL Auto-Allocate] (A4T)整合可讓您檢視[!DNL Adobe Analytics]中的報告資料，而且您可以最佳化[!DNL Analytics]中定義的自訂事件或量度。
