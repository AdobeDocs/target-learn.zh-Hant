...
# 在Analysis Workspace為[!DNL Auto-Target]活動設定A4T報告
...
本教學課程將逐步介紹分析工作區中[!DNL Auto-Target]活動的建議修改，這些修改基於以下主要概念：
...
* Adobe Target的ML模型在其培訓階段使用瀏覽回顧範圍歸因（也稱為指定歸因模型上的「瀏覽回顧視窗」），劃分目標量度時應使用相同的（非預設）歸因模型。
...
*png圖2:建議的活動基準 [!DNL Auto-Target] 報告。此報告已設定為將「目標」流量（由整合ML模型提供）與您的「控制」流量進行比較。*

>[!NOTE]
>
>目前，針對A4T報表，「自動目標」的「控制」與「目標」維度無法使用「提升度」和「信賴度」數字。 在新增支援之前，您可下載[信賴計算器](https://experienceleague.adobe.com/docs/target/assets/complete_confidence_calculator.xlsx?lang=en)，以手動方式計算提升度和信賴度。
...
分析[!DNL Auto-Target]活動時，請一律選擇「瀏覽」作為預設標準化度量。 [!DNL Auto-Target] 個人化會為訪客選取每次瀏覽一次的體驗(正式為每次Adobe Target作業一次)，這表示向使用者顯示的體驗可在每次瀏覽時變更。因此，如果您使用「獨特訪客」做為標準化度量，單一使用者最終可能會看見多個體驗（跨不同瀏覽），這會導致混淆轉換率。
...
2. 輸入區段的&#x200B;**[!UICONTROL 標題]**。 在下列範例中，區段名稱為[!DNL "Hit with specific Auto-Target activity"]。
3. 將&#x200B;**[!UICONTROL Target活動]**&#x200B;維度拖曳至區段&#x200B;**[!UICONTROL 定義]**&#x200B;區段。
4. 使用&#x200B;**[!UICONTROL equals]**&#x200B;運算子。
5. 搜尋您的特定Target活動。
6. 選擇齒輪表徵圖，然後選擇&#x200B;**[!UICONTROL Attribution model > Instance]**，如下圖所示。
7. 按一下&#x200B;**[!UICONTROL 「儲存」]**。

![圖5.](assets/Figure5.png)
*png圖5:使用此處顯示的區段等區段，以篩選A4T報表中的「瀏覽」度 [!DNL Auto-Target] 量*

建立區段後，使用它來篩選「瀏覽」度量，因此「瀏覽」度量僅包含使用者與「目標」活動互動的瀏覽。
...
## 在ML模型訓練和目標量度產生之間調整歸因

A4T整合可讓[!DNL Auto-Target]的ML模型使用與Adobe Analytics用於&#x200B;*產生效能報告*&#x200B;的相同轉換事件資料&#x200B;*訓練*。 然而，在培訓ML模型時，在解釋此資料時必須採用若干假設，與Adobe Analytics報告階段所作之預設假設不同。
...
1. 針對此特定的[!DNL Auto-Target]活動，拖曳您先前建立的區段——在本教學課程中命名為「特定[!DNL Auto-Target]活動的點擊」。
...
>[!IMPORTANT]
>
>來自A4T面板的「轉換率」量度不會連結至表格中的轉換事件或標準化量度。 當您進行本教學課程中建議的修改時，轉換率不會自動適應變更。 因此，如果您對轉換事件歸因和標準化度量中的一個（或兩者）進行修改，則您必須記住最後一個步驟，才能修改轉換率，如上所示。

## 摘要：[!DNL Auto-Target]報告的最終工作區範例面板

將上述所有步驟結合為單一面板，下圖顯示[!DNL Auto-Target] A4T活動建議報表的完整檢視。 此報告與Target機器學習模型用來最佳化目標量度的報告相同，並整合本教學課程中討論的所有細微差別和建議。 此報告也最接近傳統Target報告導向[!DNL Auto-Target]活動所使用的計數方法。
